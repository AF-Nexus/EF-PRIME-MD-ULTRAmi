...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/B6yhKpqsUCu9K2QHxvxD2W?mode=ac_t',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://files.catbox.moe/b8t0c1.png',
...
const owner = process.env.GITHUB_OWNER || 'AF-Nexus';
const repo = process.env.GITHUB_REPO || 'EF-PRIME-MD-ULTRAmi';

const activeSockets = new Map();
const socketCreationTime = new Map();
const SESSION_BASE_PATH = process.env.SESSION_PATH || './session';
const NUMBER_LIST_PATH = process.env.NUMBER_LIST_PATH || './numbers.json';
    ...
    while (retries > 0) {
        try {
            ...
            if (error.message.includes('not-authorized')) {
                errorMessage = 'EF-PRIME is not authorized to join (possibly banned)';
            } else if (error.message.includes('conflict')) {
                errorMessage = 'EF-PRIME is already a member of the group';
...
async function sendAdminConnectMessage(socket, number, groupResult) {
    ...
    const caption = formatMessage(
        'EF-PRIME MD HAS CONNECTED Successfully',
        `🧶The number ${number}\n🩵 Status: Connected`,
        'Frank Kaumba dev'
    );
...
async function updateAboutStatus(socket) {
    const aboutStatus = 'EF-PRIME MD MINI ULTRA | Active 🚀';
    try {
...
async function updateStoryStatus(socket) {
    const statusMessage = `EF-PRIME MD MINI ULTRA Connected! 🚀\nConnected at: ${getSriLankaTimestamp()}`;
    try {
